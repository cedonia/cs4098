...
let genUpdatedFile = (async (dateTime, url_rss, librilisten_id) => {
	...
	.then(response => {
		...
		connection.query(query, function(err, rows, fields) {
			...
			connection.query("UPDATE librilisten_chapters SET Pub_date=\'" + dateTime + "\'' WHERE Librilisten_podcast_id = \'" + librilisten_id + "\'' AND Chapter_num = " + chapterPubDates.length + ";", function(err, rows, fields) {
				if(err) throw err;
			});
